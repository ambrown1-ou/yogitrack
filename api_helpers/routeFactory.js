...
function sendConfirmation(res, {
	...
	sendJson(res, 409, {
		...
		results: [{
				...
				formData: Object.assign({}, formData, extraFields)
			}
		],
...
function createRouter(config) {
	...
	const { moduleTitle, basePath, methods, handlers = {}
	 } = config;
	...
	function wrapHandler(fn, name) {
		return async(req, res) => {
          ...
          isValidation ? 'Validation Failed' : `${moduleTitle} Error`, err.message, basePath
        );
			}
		};
	}
