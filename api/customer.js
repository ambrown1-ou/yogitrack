...
module.exports = createRouter({
	...
	handlers: {
		...
		async getCustomerByName(req, res) {
			...
			if (!docs.length)
				return sendError(res, 404, 'Customer Not Found', `No customers found matching that name`, BACK);
		...
		async updateCustomer(req, res) {
			...
			if (address !== undefined)
				doc.address = address ? address.trim() : '';
			if (phone !== undefined)
				doc.phone = phone ? phone.trim() : '';
			if (email !== undefined)
				doc.email = email ? email.trim() : '';
			if (preferredContactMethod)
				doc.preferredContactMethod = preferredContactMethod;
			if (dateOfBirth !== undefined)
				doc.dateOfBirth = dateOfBirth ? new Date(dateOfBirth) : undefined;
		...
		async deleteCustomer(req, res) {
			...
			const relatedAttendance = await Attendance.countDocuments({
				customerId: customerId.trim()
			});
